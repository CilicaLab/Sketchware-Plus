...
class DependencyResolver(
    ...
    companion object {
        private val DEFAULT_REPOS = """
          ...
          |    {"url": "https://maven.aliyun.com/repository/public", "name": "Aliyun Public"},
    ...
    init {
        if (Files.notExists(repositoriesJson)) {
            ...
            // Migration: Ensure Google Maven and Maven Central are present
            val existingReposText = repositoriesJson.readText()
            if (!existingReposText.contains("maven.google.com") || !existingReposText.contains("repo1.maven.org")) {
                // If important repos are missing, reset to default (simplest migration)
                // Alternatively, we could merge, but resetting ensures a clean state with new defaults.
                repositoriesJson.writeText(DEFAULT_REPOS)
            }
        }
            ...
            if (url != null) {
                repositories.add(object : Repository {
                    override fun getName(): String {
                        return (it["name"] as String?) ?: "Unknown"
                    }

                    override fun getURL(): String {
                        val name = (it["name"] as String?) ?: "Unknown"
                        val receiver = eventReciever
                        if (receiver is DependencyResolverCallback) {
                            receiver.onSearchingRepository(name)
                        }
    ...
    open class DependencyResolverCallback : EventReciever() {
        ...
        open fun onSearchingRepository(repoName: String) {}
    }
