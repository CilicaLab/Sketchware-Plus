...
import sketchware.plus.xml.XmlBuilderHelper;
...
public class yq {
    ...
    public ArrayList<SrcCodeBean> a(hC projectFileManager, eC projectDataManager, BuiltInLibraryManager builtInLibraryManager) {
        ...
        // Load XML and Java command patches
        String xmlPath = wq.b(sc_id) + "/command";
        String javaPath = wq.b(sc_id) + "/java_command";
        CommandBlock.loadCommands(xmlPath);
        CommandBlock.loadCommands(javaPath);
        ...
        for (ProjectFileBean activity : projectFileManager.b()) {
            if (!javaFiles.contains(new File(javaDir + activity.getJavaName()))) {
                String code = new Jx(N, activity, projectDataManager).generateCode(isAndroidStudioExport, sc_id);
                srcCodeBeans.add(new SrcCodeBean(activity.getJavaName(), CommandBlock.applyCommands(activity.getJavaName(), code)));
            }
        }
    ...
    public String getFileSrc(String filename, hC projectFileManager, eC projectDataManager, iC projectLibraryManager) {
        ...
        CommandBlock.x();
        CommandBlock.loadCommands(wq.b(sc_id) + "/command");
        CommandBlock.loadCommands(wq.b(sc_id) + "/java_command");
        ...
        for (ProjectFileBean file : files) {
            if (filename.equals(isJavaFile ? file.getJavaName() : file.getXmlName())) {
                if (isJavaFile) {
                    String code = new Jx(N, file, projectDataManager).generateCode(isAndroidStudioExport, sc_id);
                    return CommandBlock.applyCommands(filename, code);
                } else if (isXmlFile) {
