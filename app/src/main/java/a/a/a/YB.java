...
public class YB extends MB {
    ...
    public void onTextChanged(CharSequence charSequence, int start, int before, int count) {
        ...
        if (charSequence.toString().trim().length() < 3) {
            ...
            if (context != null) {
                b.setError(context.getString(R.string.invalid_value_min_lenth, 3));
            }
            d = false;
            return;
        }
        if (charSequence.toString().trim().length() > 100) {
            b.setErrorEnabled(true);
            if (context != null) {
                b.setError(context.getString(R.string.invalid_value_max_lenth, 100));
            }
            d = false;
            return;
        }
        ...
        for (String reservedName : reservedNames) {
            if (charSequence.toString().equals(reservedName)) {
                b.setErrorEnabled(true);
                if (context != null) {
                    b.setError(context.getString(R.string.logic_editor_message_reserved_keywords));
                }
                d = false;
                return;
            }
        ...
        if ("main".equals(charSequence.toString()) || existingNames.contains(charSequence.toString())) {
            ...
            b.setErrorEnabled(true);
            if (context != null) {
                b.setError(context.getString(R.string.logic_editor_message_variable_name_must_start_letter));
            }
            d = false;
            ...
            b.setErrorEnabled(true);
            if (context != null) {
                b.setError(context.getString(R.string.invalid_value_rule_4));
            }
            d = false;
        }
    }
