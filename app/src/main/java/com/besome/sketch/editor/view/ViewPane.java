...
import sketchware.plus.activities.resourceseditor.components.utils.ColorsEditorManager;
...
import sketchware.plus.utility.InjectAttributeHandler;
import sketchware.plus.utility.InvokeUtil;
import sketchware.plus.utility.PropertiesUtil;
import sketchware.plus.utility.FirebaseUtil;
import sketchware.plus.utility.ResourceUtil;
...
public class ViewPane extends RelativeLayout {
    ...
    private final HashMap<String, View> viewCache = new HashMap<>();

    public ViewPane(Context context) {
        super(context);
        if (FirebaseUtil.isFirebaseInitialized(context)) {
    ...
    public void updateRootLayout(String sc_id, String fileName) {
        ...
        if (rootView instanceof ItemView sy) {
            sy.setFixed(true);
            viewCache.put(currentBean.id, rootView);
        }
        if (rootLayout != null) {
            ...
            if (viewBean.getClassInfo().b("FloatingActionButton") && (imageBean = viewBean.image) != null && (str = imageBean.resName) != null && !str.isEmpty()) {
                try {
                    ...
                    if (imagePath.endsWith(".xml")) {
                        ...
                        svgUtils.loadScaledSvgIntoImageView(fab, fpu.getSvgFullPath(sc_id, viewBean.image.resName), 0.5f);
                    } else if (new File(imagePath).exists()) {
                        svgUtils.loadImage(fab, imagePath);
                    } else {
                        XmlToSvgConverter xmlToSvgConverter = new XmlToSvgConverter();
                        new Thread(() -> {
                            try {
                                xmlToSvgConverter.setImageVectorFromFile(fab, xmlToSvgConverter.getVectorFullPath(DesignActivity.sc_id, viewBean.image.resName));
                            } catch (Exception e) {
                                Log.e("ViewPane", "FAB Vector loading failed: " + e.getMessage());
                            }
                        }).start();
                    }
            ...
            try {
                if (resourcesManager.h(backgroundResource) == ProjectResourceBean.PROJECT_RES_TYPE_RESOURCE) {
                    ...
                    if (backgroundRes.endsWith(".9.png")) {
                        ...
                        svgUtils.loadBackground(view, backgroundRes);
                    }
                }
            } catch (Exception e) {
            ...
            if (resourcesManager.h(viewBean.image.resName) == ProjectResourceBean.PROJECT_RES_TYPE_RESOURCE) {
                ...
                try {
                    ...
                    if (imagelocation.endsWith(".xml")) {
                        FilePathUtil fpu = new FilePathUtil();
                        svgUtils.loadScaledSvgIntoImageView((ImageView) view, fpu.getSvgFullPath(sc_id, viewBean.image.resName), 0.5f);
                    } else if (new File(imagelocation).exists()) {
                        svgUtils.loadImage((ImageView) view, imagelocation);
                    } else {
                        XmlToSvgConverter xmlToSvgConverter = new XmlToSvgConverter();
                        new Thread(() -> {
                            try {
                                ...
                                Log.e("ViewPane", "ImageView Vector loading failed: " + e.getMessage());
                            }
                        }).start();
                    }
    ...
    public ItemView findItemViewByTag(String str) {
        View findViewWithTag = viewCache.get(str);
        if (findViewWithTag == null) {
            if (str.charAt(0) == '_') {
    ...
    public void addViewAndUpdateIndex(View view) {
        ...
        if (rootLayout != null) {
            View parentView = viewCache.get(bean.parent);
            if (parentView == null) {
                parentView = rootLayout.findViewWithTag(bean.parent);
            }

            if (parentView instanceof ViewGroup viewGroup) {
                viewGroup.addView(view, bean.index);
                viewCache.put(bean.id, view);
