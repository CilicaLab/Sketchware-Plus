package sketchware.plus.tools;

import static sketchware.plus.SketchApplication.getContext;
import static sketchware.plus.utility.PropertiesUtil.parseReferName;
...
public class ViewBeanParser {
    ...
    public ArrayList<ViewBean> parse() throws XmlPullParserException, IOException {
        ...
        Set<String> ids = new HashSet<>(reservedIds);
        Set<String> usedReservedIds = new HashSet<>();
        ...
        while (parser.getEventType() != XmlPullParser.END_DOCUMENT) {
            switch (parser.getEventType()) {
                case XmlPullParser.START_TAG -> {
                    ...
                    String attrIdName = attrId != null ? parseReferName(attrId, "/") : null;

                    // We allow reserved IDs if they are explicitly provided in the XML (e.g. restoring a skeleton)
                    // but we still prevent duplicates for non-reserved IDs.
                    String id;
                    if (attrIdName != null && (!ids.contains(attrIdName) || (reservedIds.contains(attrIdName) && !usedReservedIds.contains(attrIdName)))) {
                        id = attrIdName;
                        if (reservedIds.contains(id)) usedReservedIds.add(id);
                    } else {
