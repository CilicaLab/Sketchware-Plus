package sketchware.plus.activities.main.activities;
...
import sketchware.plus.activities.about.AboutActivity;
...
public class MainActivity extends BasePermissionAppCompatActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        binding.bottomNav.setOnItemReselectedListener(item -> {
            if (item.getItemId() == R.id.item_explore && exploreFragment != null) {
        ...
        if (savedInstanceState != null) {
            ...
            exploreFragment = (ExploreFragment) getSupportFragmentManager().findFragmentByTag(EXPLORE_FRAGMENT_TAG);
    ...
    private Fragment getFragmentForNavId(int navItemId) {
        ...
        } else if (navItemId == R.id.item_explore) {
    ...
    private void navigateToExploreFragment() {
        ...
        if (fm.findFragmentByTag(EXPLORE_FRAGMENT_TAG) == null) {
            shouldShow = false;
            transaction.add(binding.container.getId(), exploreFragment, EXPLORE_FRAGMENT_TAG);
        }
        if (shouldShow) transaction.show(exploreFragment);
        transaction.commit();
    ...
    private void allFilesAccessCheck() {
        if (Build.VERSION.SDK_INT > 29) {
            ...
            if (!optOutFile.exists() && !granted) {
                ...
                dialog.setMessage("Starting with Android 11, Sketchware Plus needs a new permission to avoid " + "taking ages to build projects. Don't worry, we can't do more to storage than " + "with current granted permissions.");
