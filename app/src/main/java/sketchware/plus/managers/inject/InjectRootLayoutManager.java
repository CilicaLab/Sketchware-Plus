package sketchware.plus.managers.inject;
...
public class InjectRootLayoutManager {
    ...
    public Root getLayoutByFileName(String name) {
        Map<String, Root> data = get();
        if (data == null) return getDefaultRootLayout();
        return data.getOrDefault(name, getDefaultRootLayout());
    }
    ...
    public Map<String, Root> get() {
        if (FileUtil.isExistFile(path)) {
            try {
                ...
                Map<String, Root> data = new Gson().fromJson(content,
